...
START_TEST(sum_matrix) {
  ...
  matrix_t res = {0};
  int status = s21_sum_matrix(&m, &mtx, &res);
...
Suite *suite_s21_sum_matrix() {
  Suite *s = suite_create("[s21_sum_matrix] Unit Test");
  TCase *tc = tcase_create("s21_sum_matrix");
  TCase *tc1 = tcase_create("s21_sum_matrix_er1");
  TCase *tc2 = tcase_create("s21_sum_matrix_er2");

  tcase_add_loop_test(tc, sum_matrix, 0, 20);
  tcase_add_test(tc1, calcul_err);
  tcase_add_test(tc2, incorrect_matr);
